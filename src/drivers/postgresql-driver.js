...
class PostgreSQLDriver extends DriverInterface {
    ...
    static async query(sql) {
        try {
            await this.connect()
            let response = await this.CONNECTION.query(sql);
            return response
        } catch (e) {
            console.log(e)
        } finally {
            let g = await this.CONNECTION.end()
        }
    }

    static async createTable(table_name) {
        return await this.query(PostgreSQLQueryBuilder.createTable(table_name))
    }
    ...
    static async insertRecord(tablename, alpha_record) {
        ...
        let insert = await this.query(PostgreSQLQueryBuilder.insertRecord(tablename, alpha_record))
        for (let a of Object.keys(inserted)) {
            alpha_record[a] = inserted[a]
            delete alpha_record[`${alpha_record[a]._tablename}_id`]
        }
        return insert[1].rows[0].currval
    }
    ...
    static async updateRecord(alpha_record) {
        ...
        let update = await this.query(PostgreSQLQueryBuilder.updateRecord(tablename, alpha_record, id))
    ...
    static async getColumns(tablename) {
        return await this.query(PostgreSQLQueryBuilder.getColumns(tablename))
    }

    static async updateColumns(tablename, updated_columns) {
        await this.query(PostgreSQLQueryBuilder.updateColumns(tablename, updated_columns))
    }

    static async createColumns(tablename, new_columns) {
        await this.query(PostgreSQLQueryBuilder.createColumns(tablename, new_columns))
    }
    ...
    static async store(alpha_record, base = true) {
        ...
        try {
            ...
            columns_db = columns_db.rows

            let { updated_columns, new_columns } = await PostgreSQLGenerator.columns(columns_db, alpha_record)
